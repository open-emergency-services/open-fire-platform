...
**Status:** Proposed *(direction accepted; specific tech choices marked open below)*
**Date:** 2026-08-21
...
stream (mayday, ADR-0002) is emitted from the Core's committed log directly. Therefore
**life-safety is independent of the read model**: a dead Replica degrades dashboards and
rich queries, not the mayday reaching a screen.
...
## Open specifics (recommendations — confirm or override)

1. **Core commit mode:** quorum-synchronous (`synchronous_standby_names = 'ANY 1 (s1,s2)'`)
   for RPO 0 with ≥ 2 standbys. *(Recommended.)*
2. **Failover tooling:** Patroni (mainstream, consensus-backed) over `pg_auto_failover`
   (lighter). *(Recommended: Patroni for production, single-node for dev.)*
3. **Projector mechanism:** Postgres **logical replication / logical decoding** off the
   Core log, with a monotonic cursor as the fallback. *(Recommended.)*
4. **Targets:** propose Core **RPO = 0**; read-model **RTO = seconds** via hot-standby
   promotion. *(Confirm the numbers.)*
5. **Ingest termination:** a thin ingest endpoint on the **stable API** writes to the Core
   (no logic), keeping the essential surface minimal. *(Recommended.)*
...
2. [ ] Build the projector: consume the Core log → materialize the incident/comms read
       model; turn today's in-memory store into that projection.
3. [ ] Point the alert stream at the Core's committed log (not the read model).
4. [ ] Mandate the writer/read-model boundary + per-service network contracts in code so
       L1 → L2 is config-only.
5. [ ] Package topologies: compose profiles / Helm values for `single-node-dev`,
       `level-1`, and `full-ha` (Core primary + 2 standbys + Patroni; hot-standby replicas).
6. [ ] Write ADR-0005 (tiered APIs) as the companion that defines the stable / regular /
